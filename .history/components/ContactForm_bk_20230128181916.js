...
export default function ContactForm() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!/^[A-Z]/.test(first)) {
      updatedErrors.first = "First name error. First letter must be capital. Must use alphabets only.";
    } else {
      updatedErrors.first = "";
    }

    if (!/^[A-Z]/.test(last)) {
      updatedErrors.last = "Last name error. Last letter must be capital. Must use alphabets only.";
    } else {
      updatedErrors.last = "";
    }
    ...
    <div className={styles.cont_form}>
      ...
      <div>
        {Object.entries(errors).map(([field, error]) => {
          if (error) {
            return <p className={styles.error}>{`${field} ${error}`}</p>
          }
        })}
      </div>
    </div>
